...
def numpy_bbox_to_image(image, bbox_list, labels=None, scores=None, class_name=[], config=None):
    ...
    for b in np.argsort(bbox_area)[::-1]:
        ...
        class_color = (CLASS_COLOR_MAP[int(class_id)]/255).tolist()
        # class_color = (0,0,0)
    
        multiplier = image.shape[0] / 500
        cv2.rectangle(image, (x1, y1), (x1 + int(multiplier*15)*len(label_name), y1 + 20), class_color, -10)
